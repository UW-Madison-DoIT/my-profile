...
public class EmergencyContactInfoResultSetExtractor implements ResultSetExtractor<ContactInformation[]> {
  ...
  public ContactInformation[] extractData(ResultSet rs) throws SQLException, DataAccessException {
    if(rs.next()) {
      ...
      try {
        ...
        value = ContactInformationMapper.convertToEmergencyContactInformation(json);
        return value;
      } catch (Exception e) {
